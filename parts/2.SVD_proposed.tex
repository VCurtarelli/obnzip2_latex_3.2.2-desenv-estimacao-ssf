...
Nós denotamos $R = \rank{\bvR{i}}$, e assumimos que $R < N$ (ou seja, $\bvR{i}$ é uma matriz de posto deficiente). Nessa situação, apenas $R$ dos valores singulares de $\bvR{i}$ são não-nulos, e portanto então $\lambda_{R+1} = \cdots = \lambda_{N} = 0$. Nesse cenário, a matriz $\pinv{[\bvR[T]{i} \bvR{i}]}$ --- onde $\pinv*{\cdot}$ denota a pseudo-inversa --- pode ser obtida como
\begin{equation}
	\begin{split}
		\pinv*{\bvR[T]{i} \bvR{i}}
